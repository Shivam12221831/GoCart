    ...
    const fetchSellerStatus = async () => {
        ...
        try {
            ...
            if(['approved', 'rejected', 'pending'].includes(data.status)){
                ...
                switch(data.status) {
                    case "approved":
                        setMessage("Your store has been approved!, you can now add products to your store from the dashboard. Redirecting to dashboard...");
                        setTimeout(() => {
                            router.push('/store');
                        }, 5000);
                        break;
                    case "rejected":
                        setMessage("Your store application has been rejected. You can contact support for more information or try submitting again.");
                        break;
                    case "pending":
                        setMessage("Your store application is currently under review. We will notify you once it's been processed. This usually takes 24-48 hours.");
                        break;
                    default:
                        break;
                }
